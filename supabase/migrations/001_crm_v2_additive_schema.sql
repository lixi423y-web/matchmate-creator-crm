...
returns trigger language plpgsql as $$
begin
  if tg_table_name='creators' and new.creator_code is null then new.creator_code='CR-'||upper(right(replace(new.id::text,'-',''),8)); end if;
  if tg_table_name='collaborations' and new.collaboration_code is null then new.collaboration_code='CO-'||upper(right(replace(new.id::text,'-',''),8)); end if;
  if tg_table_name='shipments' and new.shipment_code is null then new.shipment_code='SH-'||upper(right(replace(new.id::text,'-',''),8)); end if;
  return new;
end $$;
